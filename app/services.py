...
class EcommerceService:
    ...
    def get_order_summary(self):
        return {
            ...
            "total_amount": sum(
                order.discounted_amount or order.total_amount for order in self.orders
            ),
            "discount_codes": [order.discount_code for order in self.orders if order.discount_code],
            "total_discount_given": sum(
                order.total_amount - (order.discounted_amount or order.total_amount)
